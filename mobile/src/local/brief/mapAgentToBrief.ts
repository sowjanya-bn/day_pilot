...
export function mapAgentToBrief(
  agentReport: AgentReport | null,
): AgentBriefReflection | null {
  if (!agentReport) return null;

  const patterns = agentReport.findings
    .slice(0, 2)
    .map((finding) => finding.summary);
  const insight = agentReport.insights.length
    ? agentReport.insights[0].message
    : null;
